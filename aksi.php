...
require_once'functions.php';
    ...
    if ($mod=='login_pemilih'){
        $NIM = esc_field($_POST[NIM]);
        $pass = esc_field($_POST[pass]);

        $row = $db->get_row("SELECT * FROM tb_pemilih WHERE NIM='$NIM' AND pass='$pass'");
        if($row){
            $_SESSION[login] = TRUE;
    ...
    if($mod=='pemilih_tambah'){
        ...
        if(!$NIM || !$nama_pemilih || !$pass)
            print_msg("Field bertanda * tidak boleh kosong!");
        elseif(strlen($pass)<4 || strlen($pass)>16)
            print_msg("Password 4-16 karakter!");
        elseif($db->get_results("SELECT * FROM tb_pemilih WHERE NIM='$NIM'"))
            print_msg("No NIM sudah terdaftar!");
        else{
            $db->query("INSERT INTO tb_pemilih (NIM, nama_pemilih, Fakultas, pass) VALUES ('$NIM', '$nama_pemilih', '$Fakultas', '$pass')");
        ...
        elseif(strlen($pass)<4 || strlen($pass)>16)
            print_msg("Password 4-16 karakter!");
        elseif($db->get_results("SELECT * FROM tb_pemilih WHERE NIM='$NIM' AND id_pemilih<>'$_GET[ID]'"))
            print_msg("No NIM sudah terdaftar!");
        else{
            $db->query("UPDATE tb_pemilih SET NIM='$NIM', nama_pemilih='$nama_pemilih', Fakultas='$Fakultas', pass='$pass' WHERE id_pemilih='$_GET[ID]'");
            redirect_js("index.php?m=pemilih");
        }
